...
export default function ResultsPage({ resumeData, onNext, onPrevious }: ResultsPageProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    if (skillsBreakdown.length > 0) {
      const breakdownContent = skillsBreakdown.map((item) => `- ${item.label}: ${item.value}%`)
      sections.push({
      ...
      <div className="flex-1 py-4 sm:py-6">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 w-full space-y-4 sm:space-y-6">
          ...
          <div className="text-center space-y-3 sm:space-y-4 mb-6 sm:mb-8 animate-fade-in-up">
            <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-4 sm:py-2 bg-success/10 dark:bg-success/10 rounded-full border border-success/20 dark:border-success/20 mb-3 sm:mb-4">
              <Award className="w-3 h-3 sm:w-4 sm:h-4 text-success dark:text-success" />
              <span className="text-xs sm:text-sm font-medium text-success dark:text-success">Analysis Complete</span>
            </div>
            <div className="flex justify-center">
              <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-gradient-to-br from-success/20 dark:from-success/20 to-success/10 dark:to-success/10 rounded-full flex items-center justify-center animate-pulse border-2 border-success/30 dark:border-success/30">
                <CheckCircle2 className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-success dark:text-success" />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="card-base hover:shadow-lg sm:hover:shadow-xl transition-all duration-300 animate-fade-in-up border-t-3 sm:border-t-4 border-t-primary rounded-xl p-4 sm:p-6 bg-gradient-to-br from-primary/5 to-primary/10">
              ...
              <div className="flex flex-col items-center justify-center text-center w-full">
                <div className="text-5xl sm:text-6xl lg:text-7xl font-bold text-primary mb-3 sm:mb-4">
                  {matchPercentage}%
                </div>
                {keywordAnalysis && matchPercentage > 0 && (
                  <div className="flex items-center gap-2 bg-background/80 backdrop-blur-sm rounded-full px-3 py-1.5 sm:px-4 sm:py-2 border border-border">
                    <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 lg:w-7 lg:h-7 text-success" />
              ...
              <div className="space-y-3 sm:space-y-4">
                {topStrengths.length > 0 ? (
                  topStrengths.map((strength, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 p-2.5 sm:p-3 rounded-lg bg-secondary/5 hover:bg-secondary/10 transition-colors group"
                    >
                      ...
                      <span className="text-foreground font-medium leading-relaxed text-sm sm:text-base">
                        {strength}
                      </span>
                    </div>
                  ))
                ...
                {skillsBreakdown.map((item, i) => (
                  <div key={i} className="group">
                    <div className="flex justify-between items-center mb-2 sm:mb-3">
                      ...
                      <span className={`text-base sm:text-lg font-bold ${getMatchColor(item.value)}`}>
                        {item.value}%
                      </span>
                    </div>
            ...
            >
              ...
              <div className="space-y-2 sm:space-y-3">
                {nextSteps.map((step, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors group"
                  >
