...
const SECTION_CONFIG: Array<{
...
}> = [
  {
    ...
    empty:
      "We couldn't spot clear strengths yet. Try highlighting your impact and core expertise.",
  },
  {
    key: "weaknesses",
    ...
    empty:
      "No obvious gaps detected. Double-check that your resume addresses the target role's requirements.",
  },
  {
    key: "improvements",
    ...
    empty:
      "You're in good shape! Consider tailoring your resume to specific job descriptions for finer adjustments.",
  },
];
...
export default function AnalysisPage({
  ...
  // Get theme colors for chart - UPDATED to use CSS custom properties
  ...
  const handleDownloadSummary = () => {
    ...
    const metadata = {
      ...
      footer:
        "Generated by CVisionAI - AI-Powered Resume Analysis Tool | Confidential Report - For personal use only",
    };

    const pdfBlob = createPdfBlob(sections, metadata);
    const filename = `cv-analysis-report-${
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 w-full space-y-6">
          ...
          <div className="text-center animate-fade-in-up">
            ...
            <p className="text-sm sm:text-lg text-muted-foreground max-w-2xl mx-auto px-2 sm:px-0 leading-relaxed">
              We analyzed your resume to surface strengths, gaps, and actionable
              next steps.
            </p>
          </div>
          ...
          {isLoading && (
            <div className="card-base animate-fade-in-up border border-dashed border-primary/40 bg-primary/5 flex items-center gap-4 py-6 sm:py-8 px-4 sm:px-6 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-foreground text-base sm:text-lg mb-1">
                  Generating AI Insights...
                </p>
                <p className="text-xs sm:text-sm text-muted-foreground">
                  We're analyzing your skills, experience, and qualifications to
            ...
            <div className="space-y-6">
                        ...
                        content={
                          <ChartTooltipContent
                            formatter={(value, name) => {
                              ...
                              if (
                                nameStr.includes("Count") ||
                                nameStr === "count"
                              ) {
                                return [`${value}`, "Insight Count"];
                              }
                              if (
                                nameStr.includes("Confidence") ||
                                nameStr === "avgConfidence"
                              ) {
              ...
              {viewMode === "graph" && chartData.length === 0 && (
                <div className="card-base animate-fade-in-up border border-dashed border-muted-foreground/30 bg-muted/30 text-muted-foreground rounded-xl p-8 text-center">
                  ...
                  <p className="text-sm">
                    Switch to cards view to see detailed insights and
                    recommendations.
                  </p>
                </div>
              )}
          ...
          {!isLoading && !error && !analysis && (
            <div className="card-base animate-fade-in-up border border-dashed border-muted-foreground/30 bg-muted/30 text-muted-foreground rounded-xl p-6 sm:p-8 text-center">
              ...
              <p className="text-sm">
                We need more details in your resume to generate meaningful
                analysis. Try expanding your experience or skills sections.
              </p>
            </div>
          )}
