...
const SECTION_CONFIG: Array<{
...
}> = [
  {
    key: "strengths",
    title: "Top Strengths",
    borderLeftClass: "border-l-success dark:border-l-success",
    topBorderClass: "border-t-success dark:border-t-success",
    iconWrapperClass: "bg-success/10 dark:bg-success/10 text-success dark:text-success",
    metricClass: "text-success dark:text-success",
  ...
  },
  {
    ...
    borderLeftClass: "border-l-error dark:border-l-error",
    topBorderClass: "border-t-error dark:border-t-error",
    iconWrapperClass: "bg-error/10 dark:bg-error/10 text-error dark:text-error",
    metricClass: "text-error dark:text-error",
  ...
  },
  {
    ...
    borderLeftClass: "border-l-secondary dark:border-l-secondary",
    topBorderClass: "border-t-secondary dark:border-t-secondary",
    iconWrapperClass: "bg-secondary/10 dark:bg-secondary/10 text-secondary dark:text-secondary",
    metricClass: "text-secondary dark:text-secondary",
...
export default function AnalysisPage({ resumeData, onNext, onPrevious, onAnalysisPersist }: AnalysisPageProps) {
  ...
  const [themeColors, setThemeColors] = useState({
    success: "rgb(195, 232, 201)", // --color-success from your CSS
    error: "rgb(239, 68, 68)", // --color-error from your CSS
    secondary: "rgb(241, 172, 32)", // --color-secondary from your CSS
    primary: "rgb(65, 101, 213)", // --color-primary from your CSS
  ...
  const chartData = useMemo(() => {
    ...
    const categories = [
      {
        ...
        confidenceColor: "rgb(155, 215, 180)", // Lighter green for confidence bar
      ...
      },
      {
        ...
        confidenceColor: "rgb(254, 202, 202)", // Lighter red for confidence bar
      ...
      },
      {
        ...
        confidenceColor: "rgb(253, 224, 71)", // Lighter yellow for confidence bar
    ...
    return categories.map(({ key, label, color, confidenceColor, icon }) => {
      ...
      const avgConfidence =
        confidences.length > 0 ? Math.round(confidences.reduce((sum, conf) => sum + conf, 0) / confidences.length) : 0
  ...
  const chartConfig = useMemo(
    () => ({
      count: {
        label: "Insight Count",
        color: themeColors.chart1, // Using --color-chart-1 (primary blue)
      },
      avgConfidence: {
    ...
    return insights.map((insight, index) => (
      ...
      >
        <div className="flex gap-3 sm:gap-4">
          <div
            className={`flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center ${iconWrapperClass} group-hover:scale-110 transition-transform duration-300`}
          >
            {icon}
          </div>
            ...
            {insight.tags && insight.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {insight.tags.map((tag, i) => (
                  <span
                    key={i}
                    className="text-xs font-medium bg-muted px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-muted-foreground border border-border"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="space-y-6">
              ...
              {viewMode === "cards" && (
                <div className="grid gap-6">
                  {SECTION_CONFIG.map(
                    ({ key, title, borderLeftClass, topBorderClass, iconWrapperClass, metricClass, icon, empty }) => {
                      const insights = analysis[key] || []
                      return (
                        <div
                          key={key}
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center ${iconWrapperClass}`}
                              >
                                {icon}
                              </div>
                              <div>
                  ...
                  <ChartContainer config={chartConfig} className="h-[300px] sm:h-[400px] w-full">
                    <BarChart data={chartData}>
                      ...
                      <Legend
                        ...
                        iconSize={10}
                      />
                      <Bar yAxisId="left" dataKey="count" name="Insight Count" radius={[4, 4, 0, 0]}>
