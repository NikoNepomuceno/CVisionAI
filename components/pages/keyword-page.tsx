  ...
  return (
    <div className="space-y-6">
      <div className="mb-8 animate-fade-in-up">
        <h1 className="text-2xl sm:text-3xl font-bold text-foreground mb-2">Keyword Match Analysis</h1>
        <p className="text-sm sm:text-base text-muted-foreground">Compare your skills with job requirements</p>
      </div>

      <div className="card-base animate-fade-in-up border-t-4 border-t-primary">
        <label className="block text-xs sm:text-sm font-medium text-foreground mb-2">Paste Job Description</label>
        <textarea
          value={jobDescription}
          onChange={(e) => setJobDescription(e.target.value)}
          placeholder="Paste the job description here to analyze keyword matches..."
          className="input-base min-h-32 resize-none text-sm"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="card-base animate-fade-in-up border-t-4 border-t-secondary" style={{ animationDelay: "100ms" }}>
          <h2 className="text-base sm:text-lg font-semibold text-foreground mb-4">Your Skills</h2>
          <div className="space-y-2.5 sm:space-y-3">
            {yourSkills.length > 0 ? (
              yourSkills.map((item, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between hover:bg-primary/5 p-2 rounded transition-colors gap-2"
                >
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">No skills found in resume.</p>
            )}
          </div>
        </div>

        <div className="card-base animate-fade-in-up border-t-4 border-t-accent" style={{ animationDelay: "200ms" }}>
          <h2 className="text-base sm:text-lg font-semibold text-foreground mb-4">Job Keywords</h2>
          <div className="space-y-2">
            {jobKeywords.length > 0 ? (
              jobKeywords.map((item, i) => (
                <div
                  key={i}
                  className={`flex items-center justify-between p-2 sm:p-3 rounded-lg transition-all duration-200 hover:shadow-md gap-2 ${
                    item.matched
                ...
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${item.matched ? "bg-success" : "bg-secondary"}`}
                    />
                    <span className="text-xs sm:text-sm font-medium text-foreground truncate">{item.keyword}</span>
                    <span className="text-xs text-muted-foreground flex-shrink-0">({item.frequency}x)</span>
                  </div>
                  <button
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">
                No keywords extracted yet. Analyze a job description first.
              </p>
            )}
          </div>
        </div>
        ...
        <>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h3 className="font-semibold text-foreground mb-1 flex items-center gap-2 text-sm sm:text-base">
                  <TrendingUp className="w-5 h-5 text-primary flex-shrink-0" />
                  Overall Match Score
                </h3>
                <p className="text-xs sm:text-sm text-muted-foreground">How well your resume matches this job</p>
              </div>
              <div className="text-4xl sm:text-5xl font-bold text-primary">{matchPercentage}%</div>
            </div>
          </div>
          ...
          {missingKeywords.length > 0 && (
            <div className="card-base animate-fade-in-up border-t-4 border-t-error" style={{ animationDelay: "400ms" }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-foreground">Missing Keywords</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {missingKeywords.map((keyword, i) => (
                  <span
                    key={i}
                    className="text-xs bg-error/10 text-error px-2.5 sm:px-3 py-1 rounded-full border border-error/20"
                  >
                    {keyword}
                  </span>
                ))}
          ...
          {suggestions.length > 0 && (
            <div
              className="card-base animate-fade-in-up border-t-4 border-t-secondary"
              style={{ animationDelay: "500ms" }}
            >
              <h3 className="text-base sm:text-lg font-semibold text-foreground mb-3">Suggestions</h3>
      ...
      {!analysis && !isAnalyzing && (
        <div className="card-base text-center py-6 sm:py-8 text-muted-foreground animate-fade-in-up text-xs sm:text-sm">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between mt-8">
        ...
        <button
          ...
          disabled={!analysis}
          className="btn-primary flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed order-1 sm:order-2"
        >
          View Results
