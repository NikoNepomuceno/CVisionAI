...
export default function KeywordPage({ resumeData, onNext, onPrevious, onPersist }: KeywordPageProps) {
  ...
  const getMatchLabelColor = (percentage: number) => {
    ...
    if (percentage >= 60) return "text-secondary dark:text-secondary"
    if (percentage >= 40) return "text-primary dark:text-primary"
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                        <div
                          className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full flex-shrink-0 ${
                            item.foundInJob ? "bg-success dark:bg-success" : "bg-muted-foreground/30"
                          }`}
                        />
                  ...
                  jobKeywords.map((item, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                        <div
                          className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full flex-shrink-0 ${
                            item.matched ? "bg-success dark:bg-success" : "bg-primary dark:bg-primary"
                          }`}
                        />
              ...
              >
                <div className="flex flex-col items-center text-center gap-4 sm:gap-6">
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 w-full">
                    <div className="text-5xl sm:text-6xl lg:text-7xl font-bold text-primary dark:text-secondary text-center">
                      {matchPercentage}%
                    </div>
                    <div
                      className={`text-base sm:text-lg font-semibold text-center ${getMatchLabelColor(matchPercentage)}`}
                    >
