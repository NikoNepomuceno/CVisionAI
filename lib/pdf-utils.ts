 ...
 * Restructured PDF to match reference format with blue header, organized sections, and professional layout
 */
export function createPdfBlob(sections: PDFSection[], metadata: PDFMetadata): Blob {
  ...
  currentPageContent += `0 ${yPosition} 612 50 re\n`
  ...
  currentPageContent += `50 ${yPosition} 512 3 re\n`
  currentPageContent += `f\n`
  currentPageContent += `Q\n`
  ...
  for (const section of sections) {
    ...
    const estimatedHeight =
      sectionType === "header" ? 50 : sectionType === "section" ? 40 : sectionType === "subsection" ? 60 : 30
    ...
    if (sectionType === "header") {
      // Main header - blue background with white text
      currentPageContent += `q\n`
      currentPageContent += `${rgbToPdfColor(COLORS.primary)} rg\n`
      currentPageContent += `50 ${yPosition - 8} 512 35 re\n`
      currentPageContent += `f\n`
      currentPageContent += `Q\n`
      ...
      currentPageContent += `q\n`
      currentPageContent += `${rgbToPdfColor(COLORS.secondary)} rg\n`
      currentPageContent += `50 ${yPosition + 2} 250 2.5 re\n`
      currentPageContent += `f\n`
      currentPageContent += `Q\n`
      yPosition -= 12
    ...
    for (const line of section.content) {
      ...
      const indentX = section.type === "subsection" && section.priority ? 58 : 50
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    pdfLines.push(
      `<< /Type /Page /Parent ${pagesObjNum} 0 R /MediaBox [0 0 612 792] /Contents ${contentStreamNums[i]} 0 R /Resources << /Font << /F1 ${font1Num} 0 R /F2 ${font2Num} 0 R >> >> >>`,
    )
    pdfLines.push("endobj")
...
export function createAnalysisSections(
  ...
  sections.push({
    ...
    type: "subsection",
    content: resumeData.skills.length > 0 ? resumeData.skills.map((skill) => `- ${skill}`) : ["- No skills listed"],
  })
...
export function createFeedbackSections(
  ...
  // Group by category
  const groupedByCategory = feedbackItems.reduce(
    (acc, item) => {
